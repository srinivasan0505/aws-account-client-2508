...
func GetregionoptstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.GetRegionOptStatusrequest
		...
		url := fmt.Sprintf("%s/getRegionOptStatus", cfg.BaseURL)
		...
		var result models.GetRegionOptStatusResponse
...
func CreateGetregionoptstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_getRegionOptStatus",
		mcp.WithDescription("Retrieves the opt-in status of a particular Region."),
		...
		mcp.WithString("RegionName", mcp.Required(), mcp.Description("Input parameter: Specifies the Region-code for a given Region name (for example, <code>af-south-1</code>). This function will return the status of whatever Region you pass into this parameter. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetregionoptstatusHandler(cfg),
	}
}
