/**
 * MCP Server function for Disables (opts-out) a particular Region for an account.
 */
...
class Post_Disable_RegionMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_Disable_RegionHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("RegionName")) {
            queryParams.add("RegionName=" + args.get("RegionName"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post_disable_region" + queryString;
    ...
    public static MCPServer.Tool createPost_Disable_RegionTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getPost_Disable_RegionHandler(config));
    }
