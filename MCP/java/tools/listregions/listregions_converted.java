/**
 * MCP Server function for Lists all the Regions for a given account and their respective opt-in statuses. Optionally, this list can be filtered by the <code>region-opt-status-contains</code> parameter.
 */
...
class Post_List_RegionsMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_List_RegionsHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("RegionOptStatusContains")) {
            queryParams.add("RegionOptStatusContains=" + args.get("RegionOptStatusContains"));
        }
                
                String queryString = queryParams.isEmpty() ? "" : "?" + String.join("&", queryParams);
                String url = config.getBaseUrl() + "/api/v2/post_list_regions" + queryString;
    ...
    public static MCPServer.Tool createPost_List_RegionsTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getPost_List_RegionsHandler(config));
    }
