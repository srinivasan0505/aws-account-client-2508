/**
 * MCP Server function for Enables (opts-in) a particular Region for an account.
 */
...
class Post_Enable_RegionMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_Enable_RegionHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("RegionName")) {
            queryParams.add("RegionName=" + args.get("RegionName"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post_enable_region" + queryString;
    ...
    public static MCPServer.Tool createPost_Enable_RegionTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getPost_Enable_RegionHandler(config));
    }
